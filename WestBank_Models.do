...
outreg2 using myreg.doc, append keep(trt1 maxl month) addtext ("Grid cell FEs", Y, "Month FEs", N) 

*Model 3 adds month fixed effects
reghdfe viirs trt1 maxl i.month, cluster(pcbs_co month) absorb(cell_id)
outreg2 using myreg.doc, append keep(trt1 maxl) addtext ("Grid cell FEs", Y, "Month FEs", N) 

*Model 4 adds interaction with distance
reghdfe viirs c.trt1##c.dist_trt1 maxl i.month, cluster(pcbs_co month) absorb(cell_id)
outreg2 using myreg.doc, append keep(trt1 maxl c.trt1##c.dist_trt1) addtext ("Grid cell FEs", Y, "Month FEs", N) 
...
outreg2 using myreg.doc, append drop(i.month) addtext ("Grid cell FEs", Y, "Month FEs", N) 
